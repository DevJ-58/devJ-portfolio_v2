  ...
  export async function interrogerAxis({
    ...
    // Charger le prompt système depuis Firestore (avec fallback sur le prompt codé en dur)
    let promptSystemeFirestore = null
    let instructionsAdditionnelles = ''

    try {
      const [snapSysteme, snapAdditionnel] = await Promise.all([
        getDoc(doc(db, 'config_axis', 'prompt_systeme')),
        getDoc(doc(db, 'config_axis', 'prompt_principal')),
      ])
      if (snapSysteme.exists() && snapSysteme.data().contenu) {
        promptSystemeFirestore = snapSysteme.data().contenu
      }
      if (snapAdditionnel.exists() && snapAdditionnel.data().contenu) {
        instructionsAdditionnelles = snapAdditionnel.data().contenu
      }
    } catch(e) {
      console.warn('[serviceIA] Firestore prompt fetch error:', e)
    }

    // Utiliser le prompt Firestore s'il existe, sinon fallback sur le prompt codé en dur
    const promptSysteme = promptSystemeFirestore
      ? `${promptSystemeFirestore}\n\n${instructionsAdditionnelles ? `═══ INSTRUCTIONS ADDITIONNELLES ═══\n${instructionsAdditionnelles}` : ''}`
      : `${construirePromptSysteme(prenomVisiteur, profilVisiteur)}\n\n${instructionsAdditionnelles ? `═══ INSTRUCTIONS ADDITIONNELLES ═══\n${instructionsAdditionnelles}` : ''}`
